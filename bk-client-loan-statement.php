                                   ...
                                   <table class="accttitle" width="80%" border="1">
                                         ...
                                         <tr class="thbg">
                                           <th>Account Number:</th><th><select class="form-control" name="account_number" id="mySelect" required="required">
                                                          ...
                                                          <?php 
                                                              $sqlacc = "SELECT `Account number`, `Account name`, DATE(`Date opened`) AS date_opened FROM `tblloans` WHERE `client code` = ?";
                                        ...
                                        <div class="spacebar"></div>
                                         <table class="table table-hover">
                                               ...
                                               if(isset($_POST['statement_preview'])){
                                                $sql = "SELECT x.`system sn` ,x.`account number`,x.`client code`, `x`.`reference date`, `x`.`reference number`, `x`.`transtype`, `x`.`transaction number`, `x`.`details`, x.debit , x.credit, SUM(y.bal) balance FROM ( SELECT *,debit-credit bal FROM tblloansactivities ) x JOIN ( SELECT *,debit-credit bal FROM tblloansactivities ) y ON y.`system sn` <= x.`system sn` WHERE `x`.`account number` = ? AND `x`.`client code` = ? AND x.`reference date` BETWEEN ? AND ? GROUP BY x.`system sn` ASC ";
