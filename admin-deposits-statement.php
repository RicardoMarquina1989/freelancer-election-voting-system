            ...
            <div class="col-md-8"><div class="spacebar"></div>
                            <div class="breadcrumb1">
                                      <div class="bread-title">
                                              <h1>Deposits Statement Request</h1>
                                      </div>
                            </div>
                              ...
                              <div class="row">
                                ...
                                <table class="table table-striped table-bordered">
                                           ...
                                           <?php 
                                           $sentval = ACT_N;
                                            $sqlreq = "SELECT `d`.`ApplicationID`,`d`.`Client code`,`d`.`Date applied`, `d`.`FromDate`, `d`.`ToDate`, `c`.`Client name` FROM tbldepositsstatementsapply AS d LEFT JOIN tblclients AS c ON `d`.`Client code`=`c`.`Client code` WHERE `d`.`Sent`= ? ORDER BY `ApplicationID` ASC LIMIT 50";
